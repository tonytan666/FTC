...
public:
	double Xout[6][3];
	double Xin[6][3];//Xin[0] is current value
...
public:
	double ddx[6];
	double GetCorrAcc(double dx[6], double x[6], double FTd[6]);
private:
	struct MBKParm
	{
		double M[6][6];
...
double AdmitModel::GetCorrAcc(double dx[6], double x[6], double FTd[6])
{
	for (int i = 0;i < 6;i++)
	{
		if (i != 2 || i != 5)
...
{
public:
	void Euler2Quat(double Wx, double Wy, double Wz);
	void Quat2Euler();
	double q[4];
	double w[3];
...
void AngleTrans::Euler2Quat(double Wx, double Wy, double Wz)
{
...
int main()
{
	...
	while (StartFlag)
	{
		...
		AngleTrans1.Euler2Quat(MoveCorrVel[3], MoveCorrVel[4], MoveCorrVel[5]);
